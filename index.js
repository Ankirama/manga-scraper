...
mangascraper.initAPI = function(apikey) {
    if (mangascraper.currentAPI === null) { return false; } else {
...
mangascraper.latest = function() {
    return new Promise((resolve, reject) => {
        if (mangascraper.currentAPI.name == 'mangaeden') {
            return resolve([]);
        }
        return mangascraper.currentAPI.package.latest()
            .then(mangaList => {
